...
export const fallbackAbout: AboutPage = {
  ...
  milestones: [
    {
      year: "The start",
      copy: "Founded on a simple belief: great brands are built backstage, long before the spotlight hits.",
    },
    {
      year: "The work",
      copy: "Partnered with founders and teams across retail, tech and services to build brands that grow.",
    },
    {
      year: "The now",
      copy: "A tight creative unit delivering strategy, design and performance under one roof.",
    },
  ],
...
export async function getAboutPage(): Promise<AboutPage> {
  ...
  return {
    ...
    storyParagraphs:
      data.storyParagraphs?.length
        ? data.storyParagraphs
        : fallbackAbout.storyParagraphs,

    milestones:
      data.milestones?.length
        ? data.milestones
        : fallbackAbout.milestones,
